...
in the crosswalk — and in turning drivers' sight lines — before vehicles start
moving.
...
- **Mechanism split**: Toronto codes *how* each collision occurred, so the outcome can
  be split into configurations an LPI can physically prevent vs. ones it cannot —
...
prevent — pedestrians without right-of-way, vehicle occupants — did not move.
...
```
pip install pandas numpy scipy shapely statsmodels pyfixest matplotlib
python code/build_panel.py   # ~30 s
python code/estimate.py      # ~15 min
python code/fixes.py && python code/fixes2.py
python code/figures.py
```

`code/` holds the full pipeline, `results/` the estimates and the analysis panel,
`figures/` all charts.
